...
export default async function PullRequestPage({ params }: { params: Promise<{ owner: string; repo: string; number: string }> }) {
  ...
  return <RepoShell organization={row.organization} repository={row.repository} active="pulls">
    <header className="decision-header"><div><p className="eyebrow-simple">CHANGE #{detail.pull.number}</p><h1>{detail.pull.title}</h1><p>{detail.pull.authorName} wants to merge <code>{detail.pull.headBranch}</code> into <code>{detail.pull.baseBranch}</code></p></div><span className={`status-chip ${detail.pull.status}`}>{detail.pull.status}</span></header>
    <div className="decision-layout"><div className="decision-main">
      ...
      <section className="diff-section"><div className="section-heading compact"><div><p className="eyebrow-simple">SOURCE DIFF</p><h2>{comparison ? `${comparison.files.length} changed files` : "Diff unavailable"}</h2></div>{comparison && <div className="change-metrics"><span className="additions">+{comparison.additions}</span><span className="deletions">−{comparison.deletions}</span></div>}</div>
