...
import { gitCloneUrl, repositoryRoot, sshCloneUrl } from "@/lib/repository";
...
export default async function RepositoryPage({ params, searchParams }: { params: Promise<{ owner: string; repo: string }>; searchParams: Promise<{ import?: string }> }) {
  ...
  return <RepoShell organization={organization} repository={repository} active="code">
    ...
    <div className="repo-toolbar"><div className="branch-select"><GitBranch size={16} /><b>{repository.defaultBranch}</b><ChevronRight size={14} /></div><CloneBox url={gitCloneUrl(owner, repo)} sshUrl={sshCloneUrl(owner, repo)} /></div>
