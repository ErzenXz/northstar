...
import { getDb, wikiImports } from "@origin/db";
...
export default async function WikiPage({ params, searchParams }: { params: Promise<{ owner: string; repo: string }>; searchParams: Promise<{ page?: string }> }) {
  ...
  if (!wiki) return <RepoShell organization={row.organization} repository={row.repository} active="wiki"><EmptyState icon={<BookOpen/>} title="No wiki imported" detail="When a GitHub repository has a wiki, Origin mirrors its complete Git history during import."/></RepoShell>;
  const branch = await import("@origin/git").then(({ getDefaultBranch }) => getDefaultBranch(repositoryRoot, wiki.storageKey));
