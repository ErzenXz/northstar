...
export function TokenForm() {
  ...
  if (state.token) {
    return <div className="new-token"><Check /><div><b>Token created. Copy it now.</b><code>{state.token}</code><p>Northstar stores only its hash, so this value cannot be shown again.</p></div><button type="button" className="icon-button" onClick={() => navigator.clipboard.writeText(state.token!)} aria-label="Copy token"><Copy size={18} /></button></div>;
  }
