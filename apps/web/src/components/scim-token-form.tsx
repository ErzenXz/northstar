...
export function ScimTokenForm({ organizationId, hasToken }: { organizationId: string; hasToken: boolean }) {
  ...
  return <form action={formAction} className="form-stack">
    ...
    {state.token && <div className="new-token"><ShieldCheck /><div><b>SCIM bearer token — shown once</b><code>{state.token}</code><p>Store it in your identity provider now. Northstar keeps only a one-way hash.</p></div></div>}
