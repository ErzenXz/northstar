...
import { compareBranches, listBranches, mirrorRepository, repositoryExists, resolveRepositoryPath } from "@northstar/git";
...
const username = process.env.SEED_USERNAME ?? "northstar-demo";
const email = process.env.SEED_EMAIL ?? "demo@northstar.local";
const password = process.env.SEED_PASSWORD ?? "northstar-demo-2026";
...
if (!organization) {
  [organization] = await getDb().insert(organizations).values({
    ...
    slug: username,
    description: "A local workspace seeded for the Northstar product tour.",
  }).returning();
}
...
let [repository] = await getDb().select().from(repositories).where(eq(repositories.storageKey, `${username}/aurora.git`)).limit(1);
if (!repository) {
  [repository] = await getDb().insert(repositories).values({
    ...
    storageKey: `${username}/aurora.git`,
    visibility: "public",
    language: "TypeScript",
    topics: ["northstar", "demo", "agents"],
  }).returning();
}

if (!(await repositoryExists(repositoryRoot, repository!.storageKey))) {
  const temporary = await mkdtemp(join(tmpdir(), "northstar-seed-"));
  try {
    await mkdir(join(temporary, "src"), { recursive: true });
    await writeFile(join(temporary, "README.md"), `# Aurora\n\nA tiny project used to demonstrate Northstar's source browser, repository memory, and evidence-first agent planning.\n\n## Principles\n\n- Humans decide what ships.\n- Agents attach evidence to their work.\n- Project knowledge lives beside the source.\n`);
    await writeFile(join(temporary, "package.json"), JSON.stringify({ name: "aurora", private: true, type: "module", scripts: { check: "tsc --noEmit" } }, null, 2));
    ...
    await exec("git", ["config", "user.name", "Northstar Builder"], { cwd: temporary });
    await exec("git", ["config", "user.email", email], { cwd: temporary });
    await exec("git", ["add", "."], { cwd: temporary });
    await exec("git", ["commit", "-m", "Start Aurora on Northstar"], { cwd: temporary });
  ...
  await getDb().insert(activityEvents).values({
    ...
    detail: "Aurora is ready to explore.",
  });
...
if (!branches.includes("feature/decision-rail")) {
  const temporary = await mkdtemp(join(tmpdir(), "northstar-alpha2-seed-"));
  try {
...
const [productLabel] = await getDb().insert(labels).values({ repositoryId: repository!.id, name: "product", color: "2d63ff", description: "Changes visible to the people using Northstar" }).onConflictDoUpdate({ target: [labels.repositoryId, labels.name], set: { color: "2d63ff" } }).returning();
...
if (!existingStatus) await getDb().insert(commitStatuses).values({ repositoryId: repository!.id, sha: comparison.headSha, context: "northstar/demo-check", state: "success", description: "Seeded verification passed", creatorName: username });
