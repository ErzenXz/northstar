...
> Status: usable alpha. The current release hosts Git over smart HTTP, imports
> GitHub repositories and work records, and plans agent objectives. Isolated
> code-execution sandboxes, native merge mechanics, SSH transport, and billing
...
- Bare Git storage with clone, fetch, and authenticated push over smart HTTP
- Personal access tokens stored as one-way hashes
- One-step GitHub migration for Git history, branches, tags, issues, and PR records
...
[Cloud boundary](docs/cloud-boundary.md), and [Roadmap](docs/roadmap.md) before
working on production deployment or agent execution.
